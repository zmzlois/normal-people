...
There are some solutions to this, but not really. There is a design pattern called pimpl ([plmpl for compile-time encapsulation in C++](https://learn.microsoft.com/en-us/cpp/cpp/pimpl-for-compile-time-encapsulation-modern-cpp?view=msvc-170), [pointer to implementation](https://en.cppreference.com/w/cpp/language/pimpl.html)) that can be used to hide the implementation details entirely inside the cpp file. But it makes things more complicated and it comes with a performance penalty. 
