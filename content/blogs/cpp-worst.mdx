...
// 2. A static class members exists independently of any instance. 
class MyClass {
   static int counter; 
   static void method();
}

// 3. The third use case is a complete semantic non sequitur. Static variables and functions can only be accessed from the cpp file which they are defined in. 
static int value; 
static void helper();
```
